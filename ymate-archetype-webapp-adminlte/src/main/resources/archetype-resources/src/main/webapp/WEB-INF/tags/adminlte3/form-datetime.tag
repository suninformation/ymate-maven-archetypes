...
<div class="form-group ${_formGroupClass}"<c:if test="${not empty _formGroupStyle}"> style="${_formGroupStyle}"</c:if><c:if test="${not empty _formGroupAttrs}"> ${_formGroupAttrs}</c:if>>
    ...
    <div class="input-group date"<c:if test="${not empty _id}"> id="${_id}"</c:if> data-target-input="nearest">
        <jsp:doBody var="_bodyStr" scope="page"/>
        <input name="${_name}" type="text" class="form-control datetimepicker-input ${_class}"<c:if test="${not empty _id}"> id="${_id}_input" data-target="#${_id}"</c:if><c:if test="${not empty _placeholder}"> placeholder="${_placeholder}"</c:if><c:if test="${_disabled}"> disabled="disabled"</c:if><c:if test="${not empty _style}"> style="${_style}"</c:if><c:if test="${not empty _attrs}"> ${_attrs}</c:if><c:if test="${not empty _bodyStr}"> value="${_bodyStr}"</c:if>>
        <div class="input-group-append"<c:if test="${not empty _id}"> data-target="#${_id}"</c:if> data-toggle="datetimepicker">
